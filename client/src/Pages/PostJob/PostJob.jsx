...
const PostJob = () => {
  ...
  const handleSubmit = async (event) => {
    ...
    if (!isAuthenticated || !user?.email) {
      setErrors({ general: "You must be logged in with a valid email to post a job." });
      setLoading(false);
      return;
    }

    // Validate role (only recruiters can post jobs)
    try {
      const response = await axios.post(
      ...
      if (userData.role !== "recruiter") {
        setErrors({ general: "Only recruiters can post jobs." });
        setLoading(false);
        return;
      }
    } catch (error) {
      console.error("Error fetching user role:", error);
      setErrors({ general: "Error verifying user role. Please try again." });
      setLoading(false);
      return;
    }
...
const response = await axios.post(
  `${import.meta.env.VITE_API_URL || "http://localhost:3000/api"}/jobs`,
  {
    ...
    postedBy: user.sub || null, // Sends "google-oauth2|116099331485005907276"
    postedByEmail: user.email, // Sends "althafb0123@gmail.com"
  },
  { timeout: 5000 }
);
      console.log("Response:", response.data);
  ...
  if (!isAuthenticated) {
    return (
      <div className="container mx-auto px-4 py-8 text-center">
        <p className="text-red-600">Please log in to post a job.</p>
